...
runner.run(
  module,
  {
    ...
    'primaryToSecondaryUser is now in a transition state, user must enter Persona password - user clicks forgot password': function (
      done
    ) {
      browser.wclick(CSS['dialog'].forgotPassword, done);
    },
    'User completes password reset of converted address, enters new password on main site': function (
      done
    ) {
