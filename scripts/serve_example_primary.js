...
if (process.env['PUBLIC_URL']) {
  ...
  exampleServer.use(
    postprocess(function (req, buffer) {
      return buffer
        .toString()
        .replace(new RegExp('https://login.persona.org', 'g'), burl);
    })
  );
}

exampleServer.use(
  express.static(path.join(__dirname, '..', 'example', 'primary'), {
    redirect: false,
  })
);
...
exampleServer.get('/api/whoami', function (req, res) {
  if (req.session && typeof req.session.user === 'string')
...
exampleServer.post('/api/cert_key', function (req, res) {
  ...
  jwcrypto.cert.sign(
    { publicKey: pubkey, principal: { email: user + '@' + domain } },
