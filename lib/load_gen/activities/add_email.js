            ...
            wcli.post(
              ...
              function (err, r) {
                ...
                if (r.body.success !== true) {
                  ...
                  return cb(
                    common.error(
                      'failed to complete email addition for: ' + email,
                      null,
                      r
                    )
                  );
                ...
                wcli.get(
                  ...
                  function (err, r) {
                    ...
                    if (r.body.status !== 'complete') {
                      return cb(
                        common.error(
                          'registration_status failed during signup',
                          null,
                          r
                        )
                      );
                    ...
                    // now generate a key
                    common.authAndKey(cfg, user, context, email, function (
                      err
                    ) {
                      if (err) return cb(err);
                      common.genAssertionAndVerify(
                        cfg,
                        user,
                        context,
                        email,
