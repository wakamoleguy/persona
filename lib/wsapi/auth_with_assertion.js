    ...
    db.emailType(email, function (err, type) {
      ...
      if (type) {
        db.emailToUID(email, function (err, uid) {
          if (err) return wsapi.databaseDown(res, err);
          if (!uid)
            return res.json({ success: false, reason: 'internal error' });
      ...
      wsapi.requestToDBWriter(
        ...
        function (err, r) {
          try {
            ...
            if (!r.lastPasswordReset)
              throw 'response from dbwriter missing lastPasswordReset';
          } catch (e) {
            logger.error(
              'failed to create primary user with assertion for ' +
                email +
                ': ' +
                e
            );
            return res.json({
              success: false,
              reason: 'internal error creating account',
            });
          }

          logger.info(
            'successfully created primary acct for ' +
              email +
              ' (' +
              r.userid +
              ')'
