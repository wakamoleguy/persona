...
suite.addBatch({
  audiences: {
    ...
    'http://fakesite.com:8000 and http://fakesite.com:8000': matchesAudience(
      true
    ),
    'https://fakesite.com:9000 and https://fakesite.com:9000': matchesAudience(
      true
    ),

    'http://fakesite.com:8100 and http://fakesite.com:80': matchesAudience(
      false
    ),
    'https://fakesite.com:9100 and https://fakesite.com:443': matchesAudience(
      false
    ),
    'http://fakesite.com:80 and http://fakesite.com:8000': matchesAudience(
      false
    ),
    'https://fakesite.com:443 and https://fakesite.com:9000': matchesAudience(
      false
    ),

    'app://browser.gaiamobile.org and app://browser.gaiamobile.org:80': matchesAudience(
      true
    ),
  },
});
...
function make_basic_tests(new_style) {
  var title =
    'generating an assertion with ' + (new_style ? 'old style' : 'new style');
  var tests = {
    topic: function () {
      ...
      jwcrypto.assertion.sign(
        ...
        function (err, assertion) {
          ...
          var b = jwcrypto.cert.bundle([g_cert], assertion, new_style); // XXX IGNORED
...
function make_post_format_2_tests(new_style) {
  ...
  var tests = {
    ...
    'and submitting without proper Content-Type headers': {
      topic: function (err, assertion) {
        var cb = this.callback;
        var postArgs = querystring.stringify({
          assertion: assertion,
          audience: TEST_ORIGIN,
        });
        var req = http
    ...
    'fails with a helpful error message': function (r, err) {
        ...
        assert.strictEqual(
          resp.reason,
          'Content-Type expected to be one of: application/x-www-form-urlencoded, application/json'
        );
      },
    },
      ...
      jwcrypto.cert.sign(
        {
          ...
          principal: { email: TEST_EMAIL },
        },
        { issuedAt: new Date(), issuer: '127.0.0.1', expiresAt: expiration },
        {},
        fakeDomainKeypair.secretKey,
        this.callback
      );
    },
...
function make_incorrect_assertion_tests(new_style) {
  var title =
    'generating an assertion from a bogus cert with ' +
  ...
  var tests = {
    topic: function () {
      ...
      jwcrypto.assertion.sign(
        {},
        { audience: TEST_ORIGIN, expiresAt: expirationDate },
        newClientKeypair.secretKey,
        function (err, assertion) {
          if (err) return self.callback(err);
          var b = jwcrypto.cert.bundle([fakeCert], assertion, new_style); // XXX IGNORED
          self.callback(null, b);
        }
      );
    },
...
function make_crazy_assertion_tests(new_style) {
  var title =
    'generating a valid assertion with ' +
    (new_style ? 'new style' : 'old style');
  var tests = {
    topic: function () {
      var expirationDate = new Date(new Date().getTime() + 2 * 60 * 1000);
  ...
  'An assertion that expired a millisecond ago': {
    topic: function () {
      ...
      jwcrypto.assertion.sign(
        ...
        function (err, assertion) {
          if (err) return self.callback(err);
          var b = jwcrypto.cert.bundle([g_cert], assertion, true); // XXX IGNORED
  ...
  'An assertion with a bundled bogus certificate': {
    topic: function () {
      ...
      jwcrypto.assertion.sign(
        ...
        function (err, assertion) {
          if (err) return self.callback(err);
          var b = jwcrypto.cert.bundle([g_cert, 'bogus cert'], assertion, true); // XXX IGNORED
    ...
    'fails with a nice error': function (err, r) {
      ...
      assert.strictEqual(
        resp.reason,
        'signed object must have three parts, this one has 1'
      );
    },
  },
...
function make_other_issuer_tests(new_style) {
  var title =
    'generating an assertion from a cert signed by some other domain with ' +
    (new_style ? 'new style' : 'old style');
  var tests = {
    topic: function () {
      ...
      jwcrypto.assertion.sign(
        ...
        function (err, assertion) {
          if (err) return self.callback(err);
          var b = jwcrypto.cert.bundle([otherIssuerCert], assertion, new_style); // XXX IGNORED
          self.callback(null, b);
        }
      );
    },
    ...
    'will cause the verifier': {
      ...
      'to return a clear error message': function (err, r) {
        ...
        assert.strictEqual(
          resp.reason,
          "can't get public key for no.such.domain: " +
  ...
  'generating an assertion from a cert signed by a real (simulated) primary': {
    topic: function () {
      ...
      jwcrypto.assertion.sign(
        ...
        function (err, assertion) {
          if (err) return self.callback(err);
          var b = jwcrypto.cert.bundle([primaryCert], assertion);
          self.callback(null, b);
        }
      );
    },
    ...
    'will cause the verifier': {
      ...
      'to return a clear error message': function (err, r) {
        ...
        assert.strictEqual(
          resp.reason,
          "issuer 'example.domain' may not speak for emails from 'somedomain.com'"
        );
      },
    },
  },
  ...
  'certify the user key by example.domain for right email address': {
    topic: function () {
      ...
      jwcrypto.cert.sign(
        {
          publicKey: newClientKeypair.publicKey,
          principal: { email: 'foo@example.domain' },
        },
        {
          issuedAt: new Date(),
          issuer: 'example.domain',
  ...
  'generating an assertion with chained certs': {
          ...
          function (err, innerCert) {
            jwcrypto.assertion.sign(
              ...
              function (err, assertion) {
                ...
                var b = jwcrypto.cert.bundle([g_cert, innerCert], assertion);
                self.callback(null, b);
              }
            );
          }
    ...
    'will cause the verifier': {
      ...
      'to fail': function (err, r) {
        ...
        assert.strictEqual(
          resp.reason,
          'certificate chaining is not yet allowed'
        );
      },
    },
  },
