...
exports.process = function (req, res) {
  ...
  db.lastStaged(req.params.email, function (err, last) {
    ...
    if (last && new Date() - last < config.get('min_time_between_emails_ms')) {
      cef_logger.alert('EMAIL_LOAD', 'Throttling email staging request', req, {
    ...
    wsapi.bcryptPassword(req.params.pass, function (err, hash) {
      if (err) {
        ...
        cef_logger.alert('BCRYPT_ERROR', 'Error bcrypting password', req, {
          msg: err,
        });
      ...
      try {
        if (allowUnverified) {
          ...
          db.createUnverifiedUser(req.params.email, hash, function (
            err,
            uid,
            secret
          ) {
            if (err) {
              cef_logger.alert(
                'DB_FAILURE',
                'Cannot create unverified user; dbwriter failure',
                req,
                { msg: err }
              );
          ...
          db.stageUser(req.params.email, hash, function (err, secret) {
            if (err) {
              cef_logger.alert(
                'DB_FAILURE',
                'Cannot stage user; dbwriter failure',
                req,
                { msg: err }
              );
              return wsapi.databaseDown(res, err);
