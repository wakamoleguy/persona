  ...
  var client = {
    ...
    _runNextQuery: function () {
      ...
      slowQueryTimer = setTimeout(function () {
        if (
          ++consecutiveFailures > config.get('database.max_reconnect_attempts')
        ) {
          ...
          logger.warn(
            'cannot reconnect to mysql! ' +
              queryQueue.length +
              ' outstanding queries #fail.'
          );
          ...
          logger.warn(
            'Query taking more than ' +
              config.get('database.max_query_time_ms') +
              'ms!  reconnecting to mysql'
          );
