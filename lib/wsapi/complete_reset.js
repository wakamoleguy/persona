...
exports.process = function (req, res) {
  ...
  db.authForVerificationSecret(req.params.token, function (
    err,
    hash,
    existing_user,
    staged_password
    ...
    if (staged_password) {
      ...
      // is a password provided?
        ...
        return db.authForVerificationSecret(req.params.token, function (
          err,
          hash
        ) {
          if (err) {
            logger.warn(
              "couldn't get password for verification secret: " + err
            );
            return wsapi.databaseDown(res, err);
          }
  ...
  function postAuthentication(hash) {
    db.haveVerificationSecret(req.params.token, function (err, known) {
      ...
      db.completePasswordReset(req.params.token, hash, function (
        err,
        email,
        uid
      ) {
