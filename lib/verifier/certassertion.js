...
function verify(
  ...
  jwcrypto.cert.verifyBundle(
    ...
    function (err, certParamsArray, payload, assertionParams) {
      ...
      if (err) {
        logger.debug(
          "verification failure, audience mismatch: '" +
            assertionParams.audience +
            "' != '" +
            audience +
            "': " +
            err
      ...
      if (
        ultimateIssuer !== HOSTNAME &&
        ultimateIssuer !== domainFromEmail &&
        ultimateIssuer !== forceIssuer
      ) {
        primary.delegatesAuthority(domainFromEmail, ultimateIssuer, function (
          delegated
        ) {
          if (delegated) {
            return successCB(
