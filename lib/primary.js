...
const primaryTimeout = config.get('declaration_of_support_timeout_ms');
const secrets = require('./secrets.js');
const events = require('events');
const check = require('validator').check;
const wellKnownParser = require('./well-known-parser.js');
...
let PUBLIC_KEY;
try {
  PUBLIC_KEY = secrets.loadPublicKey();
  if (typeof PUBLIC_KEY !== 'object')
    throw 'secrets.loadPublicKey() returns non-object, load failure';
} catch (e) {
...
if (process.env.SHIMMED_PRIMARIES) {
  ...
  shims.forEach(function (shim) {
    ...
    logger.info(
      "inserted primary info for '" +
        domain +
        "' into cache, TODO point at '" +
        origin +
        "'"
    );
  });
...
function deepFetchWellKnown(
  ...
  fetchWellKnown(currentDomain, principalDomain, function (err, unparsedDoc) {
    ...
    try {
      ...
      return cb(
        "bad support document for '" + currentDomain + "': " + String(e)
      );
    }
    ...
    if (supportDoc.type === 'disabled') {
      ...
      logger.debug(
        delegationChain[delegationChain.length - 2] +
          ' delegates to ' +
      ...
      if (delegationChain.length > MAX_AUTHORITY_DELEGATIONS) {
        return cb(
          'Too many hops while delegating authority: ' +
            delegationChain.join(' > ')
        );
      }

      // recurse
      return deepFetchWellKnown(
        principalDomain,
        cb,
        currentDomain,
        delegationChain
      ...
      var msg =
        'unhandled error while parsing support document for ' + currentDomain;
      logger.error(msg);
      return cb(msg);
    }
  });
  ...
  deepFetchWellKnown(principalDomain, function (err, r) {
    ...
    if (err) {
      ...
      if (r.disabled) {
        ...
        logger.info(
          principalDomain + ' has explicitly disabled browserid support'
        );
      } else {
...
exports.verifyAssertion = function (assertion, cb) {
  ...
  var now = new Date();
  jwcrypto.cert.verifyBundle(assertion, now, getRoot, function (
    err,
    certParamsArray,
    payload,
    assertionParams
    ...
    var principal =
      certParamsArray[certParamsArray.length - 1].certParams.principal;
    try {
    ...
    if (rootIssuer !== domainFromEmail) {
      exports.delegatesAuthority(domainFromEmail, rootIssuer, function (
        delegated
      ) {
        if (delegated) {
