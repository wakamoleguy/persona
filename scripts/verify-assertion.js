...
wcli.post(
  ...
  function (err, response) {
    ...
    jwcrypto.generateKeypair({ algorithm: 'DS', keysize: 128 }, function (
      err,
      kp
    ) {
      if (err) return handleErr(err);
      ...
      wcli.post(
        ...
        function (err, r) {
          ...
          jwcrypto.assertion.sign({}, params, kp.secretKey, function (
            err,
            assertion
          ) {
            if (err) return handleErr(err);
            ...
            postToVerifier(args.a, bundle, args.v, function (err, res) {
              ...
              assert.strictEqual(
                res.issuer,
                urlparse(args.s).host,
                'issuer matches'
              );
              assert.ok(
                res.expires.toString().match(/^\d{13}$/),
                'expires is epoch'
              );
