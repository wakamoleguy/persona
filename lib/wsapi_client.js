...
exports.recentLatency = function () {
...
function summarizeRequestFromResponse(res) {
  ...
  if (res && res.client && res.client._httpMessage) {
    ...
    if (message.hasHeader('host')) {
      req.host = message.getHeader('host');
    }
...
function withCSRF(cfg, context, cb) {
  if (context.session && context.session.csrf_token)
    ...
    exports.get(cfg, '/wsapi/session_context', context, undefined, function (
      err,
      r
    ) {
      if (err) return cb(err);
      try {
        if (r.code !== 200) return cb({ what: 'http error', resp: r }); // report first error
