...
runner.run(
  ...
  {
    setup: function (done) {
      testSetup.setup({ browsers: 1, personatestusers: 1 }, function (
        err,
        fixtures
      ) {
        if (fixtures) {
    ...
    'load 123done and wait for the signin button to be visible': function (
      done
    ) {
    ...
    'load myfavoritebeer and wait for the signin button to be visible': function (
      done
    ) {
      browser
        .chain({ onError: done })
    ...
    'mfb sign in with allcaps personatestuser - signin is case insensitive': function (
      done
    ) {
      dialog.signInExistingUser(
        {
