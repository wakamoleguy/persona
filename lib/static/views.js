...
const generate_version_js = require('../generate_code_version');
...
function isProductionEnvironment() {
  return (
    ['https://login.persona.org', 'https://login.anosrep.org'].indexOf(
...
fs.readdir(resourceViews, function (err, files) {
  ...
  files.forEach(function (localeDir) {
    ['tos.ejs', 'privacy.ejs'].forEach(function (template) {
      fs.stat(path.join(resourceViews, localeDir, template), function (
        err,
        stats
      ) {
        if (!err && stats.isFile()) {
...
exports.setup = function (app) {
  ...
  app.use(function (req, res, next) {
    if (X_FRAME_ALLOWED.indexOf(req.path) !== -1) {
      ...
      if (
        req.path.substring(1, len) === req.lang &&
  ...
  app.get('/', function (req, res) {
    renderCachableView(req, res, 'index.ejs', {
      title: _('A Better Way to Sign In'),
      fullpage: true,
    });
  });
  ...
  app.get('/about', function (req, res) {
    renderCachableView(req, res, 'about.ejs', {
      title: _('About'),
      fullpage: false,
    });
  });
  ...
  var robots = fs
    .readFileSync(path.join(__dirname, '..', '..', 'resources', 'robots.txt'))
    .toString();
  ...
  var dntPolicy = fs
    .readFileSync(
      path.join(__dirname, '..', '..', 'resources', 'dnt-policy.txt')
    )
    .toString();
  ...
  app.get('/reset_password', function (req, res) {
    renderCachableView(req, res, 'reset_password.ejs', {
  ...
  var humans = fs
    .readFileSync(path.join(__dirname, '..', '..', 'CONTRIBUTORS'))
    .toString();
  ...
  if (isProductionEnvironment()) {
    ...
    app.get(/^\/test\/(?:index.html)?$/, function (req, res) {
      res.render('test.ejs', {
        title: 'Mozilla Persona QUnit Test',
        layout: false,
      });
    });
    ...
    app.get('/i18n_test', function (req, res) {
      renderCachableView(req, res, path.join(testPath, 'i18n_test.ejs'), {
        layout: false,
        title: 'l10n testing title',
      });
    });
    app.get('/i18n_fallback_test', function (req, res) {
      renderCachableView(
        req,
        res,
        path.join(testPath, 'i18n_fallback_test.ejs'),
        { layout: false, title: 'l10n testing title' }
      );
    });

    // /common/js/templates.js is dynamically built each time
    var templates = require('../templates');
    var dialogTemplatesPath = path.join(
      __dirname,
    ...
    var sitePartialTemplatesPath = path.join(
      __dirname,
      '../../resources/views/partial'
    );
    var siteLayoutTemplatePath = path.join(
      __dirname,
    ...
    app.get('/test/mocks/site-templates.js', function (req, res) {
      ...
      siteTemplates += templates.generate(siteLayoutTemplatePath, 'layout/');
      res.setHeader('Cache-Control', 'public, max-age=0');
      res.setHeader('Access-Control-Allow-Origin', config.get('public_url'));
      res.setHeader('Vary', 'Accept-Encoding,Accept-Language');
      res.setHeader('Content-Type', 'application/javascript');
  ...
  const REDIRECTS = {
    '/developers': {
      code: 302,
      url: 'https://developer.mozilla.org/docs/persona',
    },
    '/signin': { code: 301, url: '/' },
    '/forgot': { code: 301, url: '/' },
    '/signup': { code: 301, url: '/' },
    '/test': { code: 301, url: '/test/' },
  };
  ...
  let publicKey;
  try {
    publicKey = secrets.loadPublicKey();
  } catch (e) {
