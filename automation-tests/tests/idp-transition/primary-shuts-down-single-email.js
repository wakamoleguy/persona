...
runner.run(
  module,
  {
    ...
    'Verify testUser email': function (done) {
      ...
      restmail.getVerificationLink(testUser, function (
        err,
        token,
        verificationURL
      ) {
    ...
    'Check testUser is logged in': function (done) {
      browser.wtext(CSS['123done.org'].currentlyLoggedInEmail, function (
        err,
        text
      ) {
        done(err || assert.equal(text, testUser));
    ...
    'Verify noAuthTestUser email': function (done) {
      ...
      restmail.getVerificationLink(noAuthTestUser, function (
        err,
        token,
        verificationURL
      ) {
