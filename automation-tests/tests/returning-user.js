...
runner.run(
  ...
  {
    setup: function (done) {
      testSetup.setup({ browsers: 1, restmails: 1, testidps: 1 }, function (
        err,
        fix
      ) {
        if (fix) {
    ...
    'follow link, wait for redirect, secondary should be displayed': function (
      ...
      browser
        ...
        .wclick(
          CSS['persona.org'].signInForm.finishButton,
          secondary.split('@')[0]
        )
    ...
    'check first radio is not selected': function (done) {
      browser.wgetAttribute(CSS['dialog'].firstEmail, 'selected', function (
        err,
        val
      ) {
    ...
    'check second radio is not selected': function (done) {
      browser.wgetAttribute(CSS['dialog'].secondEmail, 'selected', function (
        err,
        val
      ) {
        done(err || assert.ok(!val));
    ...
    'sign in using primary, sign out, reload, click sign in, verify primary is selected': function (
      done
    ) {
      browser
        .chain({ onError: done })
