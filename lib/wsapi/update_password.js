    ...
    bcrypt.compare(req.params.oldpass, hash, function (err, success) {
      ...
      wsapi.bcryptPassword(req.params.newpass, function (err, hash) {
        if (err) {
          ...
          logger.error(
            'error bcrypting password for password update for user ' +
              req.session.userid,
            err
          );
          return res.json({ success: false });
        ...
        db.updatePassword(req.session.userid, hash, passwordChanged, function (
          err
        ) {
          var success = true;
          if (err) {
            logger.error(
              'error updating bcrypted password for user ' + req.session.userid,
              err
            );
