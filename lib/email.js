...
var emailTransport = emailer.createTransport('SMTP', getEmailTransportConfig());
...
function getEmailTransportConfig() {
  ...
  if (smtp_params && smtp_params.host) {
    ...
    logger.info('delivering email via SMTP host: ' + emailTransportConfig.host);
    ...
    if (smtp_params.user) {
      ...
      logger.info(
        'authenticating to email host as ' + emailTransportConfig.auth.user
      );
    }
  }
...
function compileTemplates() {
  Object.keys(templates).forEach(function (type) {
    ...
    if (templates[type].templateHTML) {
      var templateHTMLPath = path.join(
        TEMPLATE_PATH,
        templates[type].templateHTML
      );
...
function getMailOptions(
  ...
  var templateArgs = getTemplateArgs(
    verificationUrl,
    site,
    langContext,
    backgroundColor,
