...
exports.store = function (kpi_data, cb) {
  try {
    ...
    logger.debug(
      'sending request to KPI backend: ' + config.get('kpi.backend_db_url')
    );
...
function onResponse(cb, res) {
  if (res.statusCode !== 201) {
    logger.warn(
      'KPI Backend (or proxy) response code is not 201: ' + res.statusCode
    );
...
function roundTimestamps(kpi_data) {
  ...
  und.each(kpi_data, function (kpi) {
