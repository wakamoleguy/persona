...
exports.process = function (req, res) {
  db.userOwnsEmail(req.session.userid, req.params.email, function (err, owned) {
    ...
    if (!owned)
      return httputils.badRequest(res, 'that email does not belong to you');
    ...
    db.emailIsVerified(req.params.email, function (err, verified) {
      if (!verified && !req.params.allowUnverified)
        return httputils.forbidden(res, 'that email requires (re)verification');
