...
const forward = require('./http_forward.js').forward;
...
const i18n_check = require('./i18n_client_check');
const db = require('./db');
...
const COOKIE_SECRET = secrets.hydrateSecret(
  'browserid_cookie',
  config.get('var_path')
);
...
function authenticateSession(options, cb) {
  ...
  function withPasswordReset(err, lastPasswordReset) {
    ...
    if (lastPasswordReset === undefined)
      return cb(
        new Error('authenticateSession called with undefined lastPasswordReset')
      );
    ...
    if (
      ...
      logger.info(
        "not resetting cookies to 'assertion' authenticate a user who is already password authenticated"
      );
    } else {
      if (duration_ms) {
...
function checkCSRF(req, resp, next) {
  ...
  if (req.session === undefined || typeof req.session.csrf !== 'string') {
    logger.warn(
      'POST calls to /wsapi require a cookie to be sent, this user may have cookies disabled'
    );
  ...
  if (
    ...
    logger.warn(
      'CSRF validation failure, token mismatch. got:' + b + ' want:' + s
    );
...
function checkExpiredSession(req, resp, next) {
  ...
  if (req.session === undefined) {
    logger.warn(
      'calls to /wsapi require a cookie to be sent, this user may have cookies disabled'
    );
    return httputils.forbidden(resp, 'no cookie');
  }
...
function allAPIs() {
  ...
  fs.readdirSync(path.join(__dirname, 'wsapi')).forEach(function (f) {
    ...
    if (f.length <= 3 || f.substr(-3) !== '.js' || f.substr(0, 1) === '.')
      return;
...
exports.requestToDBWriter = function (opts, cb) {
  if (!exports.forwardWritesTo) {
    throw new Error(
      "cannot forward request to dbwriter, I don't know her" + 'url'
    );
  }
        ...
        res.on('end', function () {
          try {
            if (res.statusCode !== 200)
              throw 'non-200 response: ' + res.statusCode;
            cb(null, {
              headers: res.headers,
...
exports.setup = function (options, app) {
  ...
  app.use(WSAPI_PREFIX, function (req, resp, next) {
    ...
    // the request.
    if (
      !Object.prototype.hasOwnProperty.call(wsapis, operation) ||
      ...
      if (
        operation !== 'fake_verification' ||
        !process.env.BROWSERID_FAKE_VERIFICATION
      )
  ...
  app.use(WSAPI_PREFIX, cookieParser());
  app.use(WSAPI_PREFIX, (req, _res, next) => {
    if (req.text) {
      try {
        if (
        ...
        console.log('Error parsing JSON: ', req.text);
  ...
  Object.keys(all).forEach(function (operation) {
    try {
      ...
      if (
        (options.only_write_apis && !api.writes_db && operation !== 'ping') ||
      ...
      if (api.args) {
        ...
        wsapis[operation].validate = function (req, res, next) {
          next();
        };
      }
    } catch (e) {
...
exports.routeSetup = function (app, options) {
  ...
  app.use(WSAPI_PREFIX, function (req, resp, next) {
    ...
    var destination_url =
      (api.writes_db ? options.write_url : options.read_url) + destination_path;

    forward(destination_url, req, resp, function (err) {
      if (err) {
