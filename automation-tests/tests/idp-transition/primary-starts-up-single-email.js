...
const secondary = require('../../../tests/lib/secondary.js');
...
runner.run(
  module,
  {
    ...
    //
    'authenticate to persona with our username and password as a secondary': function (
      done
    ) {
      browser
        .chain()
    ...
    'enable primary support for the transition when authed test': function (
      done
    ) {
    ...
    // now a secondary
    //
    'enable primary support for the transition when not authed test': function (
      done
    ) {
      testidp.enableSupport(done);
    },
    'setup for the second test - log out and enter the dialog, and logout of persona': function (
      done
    ) {
      browser
        .chain()
    ...
    //
    'log in again with the transitioned secondary email address': function (
      done
    ) {
      browser
        .chain()
