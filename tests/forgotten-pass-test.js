...
suite.addBatch({
  'create second account': {
    topic: function () {
      wsapi
        .post('/wsapi/complete_email_confirmation', { token: token })
        .call(this);
    },
    'account created': function (err, r) {
      assert.equal(r.code, 200);
...
suite.addBatch({
  'reset status': {
    topic: wsapi.get('/wsapi/password_reset_status', {
      email: 'first@fakeemail.com',
    }),
...
suite.addBatch({
  'address_info for first': {
    ...
    "reports the email is known (which implies it's also verified)": function (
      err,
      r
    ) {
      r = JSON.parse(r.body);
...
suite.addBatch({
  'reverify status': {
    topic: function () {
      wsapi
        .get('/wsapi/email_reverify_status', { email: 'second@fakeemail.com' })
        .call(this);
    },
