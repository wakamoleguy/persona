...
function noSuchWindowErr(errText) {
  ...
  return (
    /Window not found/.test(errText) || // Firefox
    /NoSuchWindow/.test(errText) || // Chrome && Safari
    (/"status": 13/.test(errText) &&
      /Unable to find element with css selector/.test(errText)) || // IE
    /The window could not be found/.test(errText)
  ); // Chrome after July 2013
}
...
wd.prototype.newSession = function (opts, cb) {
  ...
  self.init(opts, function (err) {
    ...
    self.setImplicitWaitTimeout(timeouts.DEFAULT_IMPLICIT_WAIT_MS, function (
      err
    ) {
      if (err) return cb(err);
