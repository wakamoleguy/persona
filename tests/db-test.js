...
suite.addBatch({
  'checkAuth returns': {
    ...
    "zero failed auth tries where there aren't any": function (
      err,
      r,
      failedAuthTries
    ) {
  ...
  'getting a UID': {
    ...
    'then staging an email': {
      ...
      then: {
        topic: function (err, secret) {
          var cb = this.callback;
          db.isStaged('lloyd@somewhe.re', function (err, r) {
            cb(secret, r);
          });
        },
        'makes it visible via isStaged': function (sekret, r) {
          assert.isTrue(r);
        },
        'lets you verify it': {
          // eslint-disable-next-line
          ...
          'and isStaged': {
            topic: function () {
              db.isStaged('lloyd@somewhe.re', this.callback);
            },
  ...
  "noting that we've seen an IDP": {
    ...
    'and then checking if that IDP is known': {
      ...
      'returns a recent date object': function (err, lastSeen) {
        ...
        assert(
          delta < 1000,
          'idp was not seen in the last second (' + delta + 'ms ago)'
        );
        firstSeen = lastSeen;
      },
