...
exports.process = function (req, res) {
  function fail(reason, code) {
    ...
    cef_logger.warn('AUTH_FAILURE', 'User authentication failed', req, {
      msg: reason,
    });
    logger.debug(
      ...
      bcrypt.compare(req.params.pass, hash, function (err, success) {
        ...
        if (err) {
          if (err.indexOf('exceeded') !== -1) {
            ...
            logger.warn(
              'max load hit, failing on auth request with 503: ' + err
            );
            res.status(503);
          ...
          cef_logger.alert(
            'BCRYPT_ERROR',
            'Internal error on bcrypt.compare',
            req,
            { msg: err }
          );
...
function incrementAuthCount(req, res, uid) {
  ...
  var preq = m
    .request(
      {
        ...
        port: u.port,
        path:
          '/wsapi/increment_failed_auth_tries?' +
          querystring.stringify({ userid: uid }),
        method: 'GET',
      ...
      function (pres) {
        pres.on('end', function () {
          if (pres.statusCode !== 200) {
            cef_logger.alert(
              'DB_FAILURE',
              'Cannot increment failed authentication attempts',
              req,
              { suser: uid, msg: pres.statusCode }
            );
            logger.error(
              'failed to increment failed authentication attempts for ' +
                uid +
                ' dbwriter returns ' +
                pres.statusCode
            );
            ...
            logger.info(
              'incremented authentication failure count for user ' + uid
            );
          }
        });
      }
    ...
    .on('error', function (e) {
      ...
      logger.error(
        'cannot increment failed authentication attempts for ' + uid + ': ' + e
      );
    });
  preq.end();
}
...
function resetFailedAuthCount(req, res, uid) {
  ...
  var preq = m
      ...
      function (pres) {
        pres.on('end', function () {
          if (pres.statusCode !== 200) {
            cef_logger.alert(
              'DB_FAILURE',
              'Cannot clear failed authentication attempts',
              req,
              { suser: uid, msg: pres.statusCode }
            );
            logger.error(
              'failed to clear authentication attempts for ' +
                uid +
                ' dbwriter returns ' +
                pres.statusCode
            );
    ...
    .on('error', function (e) {
      ...
      logger.error(
        'failed to clear authentication attempts for ' + uid + ': ' + e
      );
    });
  preq.end();
}
...
function updateHash(req, res, uid, hash) {
  cef_logger.warn('AUTH_UPDATE', 'Updated password for user', req, {
    suser: uid,
  });
  ...
  var preq = m
      ...
      function (pres) {
        pres.on('end', function () {
          if (pres.statusCode !== 200) {
            cef_logger.alert(
              'DB_FAILURE',
              'Cannot update password rounds; dbwriter error',
              req,
              { suser: uid, msg: pres.statusCode }
            );
            logger.error(
              'failed to update bcrypt rounds of password for ' +
                uid +
                ' dbwriter returns ' +
                pres.statusCode
            );
    ...
    .on('error', function (e) {
      cef_logger.alert('AUTH_FAILURE', 'Error updating password rounds', req, {
        suser: uid,
        msg: e,
      });
      logger.error(
        'failed to update bcrypt rounds of password for ' + uid + ': ' + e
      );
    });
