        ...
        if (checked === count) {
          if (notOk.length === 0) {
            try {
              ...
              logger.error(
                'Exception caught in heartbeat handler: ' + e.toString()
              );
              ok(false, e);
            }
          } else {
            logger.warn(
              'heartbeat failed due to dependencies - ' + notOk.join(', ')
            );
            ok(false, '\n' + notOk.join('\n') + '\n');
          }
        }
...
var check = (exports.check = function (url, cb) {
  if (typeof url === 'string') url = urlparse(url).normalize().validate();
    ...
    throw 'url string or object required as argumnet to heartbeat.check';
  ...
  var req = require(url.scheme).get(
    ...
    function (res) {
      ...
      if (res.statusCode === 200) cb(null, true);
        ...
        logger.warn(
          'heartbeat failure: non-200 response from ' +
            shortname +
            '.  fatal! (' +
            res.statusCode +
            ')'
  ...
  req.on('error', function (e) {
    clearTimeout(timeoutHandle);
    logger.warn(
      "heartbeat failure: can't communicate with " +
        shortname +
        '.  fatal: ' +
        e
    );
