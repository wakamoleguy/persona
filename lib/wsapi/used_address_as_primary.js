    ...
    primary.checkSupport(domain, function (err, r) {
      ...
      if (err) {
        logger.error(
          '"' +
            domain +
            '" primary support is misconfigured, falling back to secondary: ' +
            err
        );
        notPrimary = true;
      } else if (r && r.urls) {
        db.updateEmailLastUsedAs(email, 'primary', function (err) {
          if (err)
            return httputils.serverError(res, 'Unable to update database');
      ...
      if (notPrimary)
        return httputils.badRequest(res, 'email is not a primary');
    });
  });
};
