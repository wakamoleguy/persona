...
suite.addBatch({
  'All resources expand': {
    ...
    'and locale specific css bundles contain the same number of assets as the non-localized bundle': function (
      files
    ) {
      ...
      assert.equal(
        files['/production/ar/dialog.css'].length,
        numDialogCSSFiles
      );
      assert.equal(
        files['/production/de/dialog.css'].length,
        numDialogCSSFiles
      );
      assert.equal(
        files['/production/fr/dialog.css'].length,
        numDialogCSSFiles
      );
    },

    'and each localized asset has a per-locale entry in the list of files': function (
      files
    ) {
      var res = resources.resources;
    ...
    'and component assets are localized if needed': function (files) {
      ...
      nonLocalizedResource.forEach(function (nonLocalizedAsset, index) {
        ...
        if (nonLocalizedAsset.indexOf(':locale') > -1) {
          ...
          var localizedRegExp = new RegExp(
            nonLocalizedAsset
