...
const logger = require('./logging/custom_logger');
...
var conf = (module.exports = convict({
  ...
  public_verifier_url: {
    doc:
      'The publically visible URL where incoming verification requests are handled',
    format: String,
    default: '',
  ...
  max_authentication_attempts: {
    doc:
      'The number of times a user may fail to authenticate before their account is locked',
    format: 'int',
    default: 10,
  ...
  database: {
    ...
    max_query_time_ms: {
      ...
      default: 5000,
      doc:
        "The maximum amount of time we'll allow a query to run before considering the database to be sick",
    ...
    max_reconnect_attempts: {
      ...
      default: 1,
      doc:
        "The maximum number of times we'll attempt to reconnect to the database before failing all outstanding queries",
    },
  },
  smtp: {
    host: {
  ...
  kpi: {
    ...
    backend_db_url: {
      doc:
        'URL of KPiggyBank service to send Key Performance Indicator data to',
      format: String,
    ...
    backend_sample_rate_per_agent: {
      doc:
        'A mapping of User Agent matches that have different rates than standard.',
      format: Object,
      default: {},
  ...
  bcrypt_work_factor: {
    doc:
      'How expensive should we make password checks (to mitigate brute force attacks) ?  Each increment is 2x the cost.',
  ...
  idp_offline_grace_period_ms: {
    doc:
      'How long should an IDP be offline before we fallback to secondary and start issuing certificates for her.',
    format: 'duration',
  ...
  maximum_event_loop_lag: {
    doc:
      'The maximum number of milliseconds the event loop may be behind before we return 503 responses',
    format: 'duration',
    default: 40,
  },
  ...
  max_compute_processes: {
    doc:
      'How many computation processes will be spun.  Default is good, based on the number of CPU cores on the machine.',
    format: 'int',
    default: 0,
  ...
  max_compute_duration: {
    doc:
      "What is the longest (in seconds) we'll let the user wait before returning a 503?",
    format: 'duration',
    default: 10,
  },
  ...
  enable_code_version: {
    doc:
      "When enabled, will cause a 'code version' to be returned to frontend code in `/wsapi/session_context` calls",
    format: Boolean,
    default: false,
  },
  min_time_between_emails_ms: {
    doc:
      "What is the most frequently we'll allow emails to be sent to the same user?",
    format: 'int',
    default: 60000,
  ...
  declaration_of_support_timeout_ms: {
    doc:
      "The amount of time we wait for a server to respond with a declaration of support, before concluding that they are not a primary. Needs to be shorter than the dialog's time_until_delay param to avoid xhr-delay UI.",
    format: 'duration',
    default: 8000,
  ...
  proxy_idps: {
    doc:
      'A mapping of domain names to urls, which maps popular email services to shimmed IDP deployments.',
    format: Object,
    default: {},
  ...
  measure_dom_loading: {
    doc:
      'Measure the time it takes the DOM to start loading by embedding an inline script in the document HEAD',
    format: Boolean,
    default: false,
  ...
  forcible_issuers: {
    doc:
      'Hostnames which this Persona instance will issue identies for. Used in b2g only.',
    format: Array,
...
if (
  ['browserid', 'router', 'static', 'verifier', 'keysigner'].indexOf(
    conf.get('process_type')
  ) !== -1
) {
...
var prefix = urlparse(
  conf.get('public_static_url') + '/' + conf.get('cachify_prefix')
);
...
module.exports.performSubstitution = function (app) {
  if (
    conf.get('public_url') !== 'https://login.persona.org' ||
    ...
    app.use(
      postprocess(function (req, buffer) {
        return buffer
          .toString()
          .replace(
            new RegExp('https://login.persona.org', 'g'),
            conf.get('public_url')
          )
          .replace(
            new RegExp('https://firefoxos.persona.org', 'g'),
            conf.get('firefoxos_url')
          )
          .replace(
            new RegExp('https://static.login.persona.org', 'g'),
            conf.get('public_static_url')
          )
          .replace(
            new RegExp('https://verifier.login.persona.org', 'g'),
            conf.get('public_verifier_url')
          );
      })
    );
  }
